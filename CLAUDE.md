...
### Intro splash (`src/components/intro`)
Full-bleed ink curtain on first visit: the lockup ("An AI Engineering / **Portfolio** /
of / Gaurav Gupta") rises in, a blue marker circle draws itself around the wordmark,
holds ~2.6s, then the curtain **slides down** to reveal the site.
- **Line boil** — every hand-drawn stroke ships as 3 jittered variants (`scribble.ts`,
  generated by `scripts/gen-scribble.py`; re-run it to retune). Cycling them at 80ms
  (`--boil-step`) fakes frame-by-frame cel animation. This is what sells the effect —
  without it the card reads as a static poster.
- **No flash** — `INTRO_BOOTSTRAP` in `layout.tsx` runs before first paint and sets
  `html[data-intro="play"]`; the curtain is `display:none` by default, so returning /
  reduced-motion / no-JS visitors never see it. `<html>` needs `suppressHydrationWarning`
  or React repairs the attribute and restarts the animation from zero.
- Plays once per session (`sessionStorage`), skips on any input, and holds the exit until
  `document.fonts.ready` (max 1.2s) so the hero is painted when it's uncovered.
