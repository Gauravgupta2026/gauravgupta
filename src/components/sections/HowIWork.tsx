...
function StageCardBody({ stage }: { stage: WorkStage }) {
  return (
    <div className="shader-card w-full rounded-[20px] border border-white/10 p-[26px] shadow-[0_40px_80px_rgba(0,0,0,.55)]">
      <p className="m-0 text-pretty font-display text-[20px] font-light leading-[1.3] text-white md:text-[18px]">
        {stage.call}
      </p>
        ...
        {workStages.map((s, i) => {
          ...
          return (
            <div key={s.num} className="relative">
              ...
              >
                <span className="font-mono text-[13px] leading-[37px] text-faint sm:text-[13px] md:text-[19px]">
                  {s.num}
                </span>
                <span className="font-body text-[17px] tracking-[-0.02em] text-white sm:text-[19px] md:text-[29px]">
                  {s.title}
                </span>
              </button>
