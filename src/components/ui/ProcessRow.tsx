...
export function ProcessRow({ process }: { process: Process }) {
  return (
    <div className="grid grid-cols-[40px_1fr] gap-[20px] border-t border-ink/15 py-[30px] pr-[24px] transition-colors duration-[350ms] last:border-b hover:bg-blue-tint md:grid-cols-[54px_1fr] md:gap-[24px]">
      <span className="font-display text-[24px] leading-none text-blue">
        {process.num}
      </span>
      <div>
        <div className="mb-[10px] font-mono text-[8px] uppercase tracking-[0.2em] text-mute">
          {process.kicker}
        </div>
        <h4 className="m-0 mb-[10px] font-display text-[18px] font-medium text-ink">
          {process.title}
        </h4>
